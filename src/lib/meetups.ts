...
async function fetchRemoteMeetups(): Promise<WebsiteMeetup[]> {
  const response = await fetch(new URL(MEETUPS_API_PATH, EVENTS_MANAGEMENT_ORIGIN), {
  ...
  console.info(
    `[meetups] Loaded ${parsed.data.data.length} published meetups from ${EVENTS_MANAGEMENT_ORIGIN.origin}.`,
  );
  return parsed.data.data.map((meetup) => mapPublicMeetup(meetup, EVENTS_MANAGEMENT_ORIGIN));
}
